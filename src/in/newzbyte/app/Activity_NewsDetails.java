...
//			hdg.setVisibility(View.GONE);
			...
			ImageView img = (ImageView)singleNewsView.findViewById(R.id.img_newsimage);
			img.setTag(R.id.img_newsimage,Integer.valueOf(0));
			...
			txt.setText(objNews.getContentSpan());
			 ...
			 txt.getViewTreeObserver().addOnGlobalLayoutListener( new OnGlobalLayoutListener() { 
		            ...
		            public void onGlobalLayout() { 
		                ...
		                // restore the margin 
		                txt_lp.setMargins(0, 0, 0, 0); 
		                SpannableString spanS =  new  SpannableString ( Html.fromHtml(objNews.getContent()) );
...
//					hdg.setText(object_SubNewsItem.getNewsHeadingSpan());
				...
				subimg.setTag(R.id.img_newsimage,Integer.valueOf(i));
				final int imgwdth = (Globals.getScreenSize(this).x-20)*3/5;
				int imghgt = (int) (imgwdth*object_SubNewsItem.getImageRatio());
				RelativeLayout.LayoutParams subimg_lp = new RelativeLayout.LayoutParams(imgwdth,imghgt);
				subimg.setLayoutParams(subimg_lp);
				
				//Globals.loadImageIntoImageView(subimg,object_SubNewsItem.getNewsImagePath(), this,R.drawable.loading_image_small,R.drawable.no_image_small);
				Globals.loadImageIntoImageView(subimg, object_SubNewsItem.getNewsImagePath(),this,R.drawable.loading, R.drawable.loading,imghgt,imgwdth);
				final TextView subtxt = (TextView)singleSubNewsView.findViewById(R.id.txt_newscontent);
				int subwidthSpec = MeasureSpec.makeMeasureSpec(LayoutParams.MATCH_PARENT, MeasureSpec.EXACTLY);
				int subheightSpec = MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED);
				subtxt.measure(subwidthSpec, subheightSpec);
				
				final int subrows = Globals.getTxtViewRowsByImgHgt(subtxt.getMeasuredHeight(),(int)(subtxt.getLineSpacingMultiplier()+subtxt.getLineSpacingExtra()),imghgt);
				
				final RelativeLayout.LayoutParams subtxt_lp = new RelativeLayout.LayoutParams(subtxt.getLayoutParams());
				subtxt_lp.setMargins(imgwdth+6, 0, 0, 0);
				subtxt.setLayoutParams(subtxt_lp);
				subtxt.setTypeface(tf);
				subtxt.setText(object_SubNewsItem.getNewsContentSpan());
				
				
				subtxt.getViewTreeObserver().addOnGlobalLayoutListener( new OnGlobalLayoutListener() { 
					 
		            public void onGlobalLayout() { 
		 
		                int linesCount = subtxt.getLayout().getLineCount();
		                // restore the margin 
		                subtxt_lp.setMargins(0, 0, 0, 0); 
		                SpannableString spanS =  new  SpannableString (object_SubNewsItem.getNewsContentSpan());
		 
		                if (linesCount <= subrows) {
		                	//Log.i("jaspal","lines count less than Rows,linescount:"+linesCount);
		                    //spanS.setSpan(new MyLeadingMarginSpan2(lines, width), 0, spanS.length(), 0);
		                    spanS.setSpan(new Custom_LeadingMarginSpace(subrows, imgwdth+6),0, spanS.length(),0);
		                    subtxt.setText(spanS);
		                } 
		                else 
		                { 
		                	//Log.i("jaspal","lines count greater than Rows,linescount:"+linesCount);
		                    // find the breakpoint where to break the String. 
		                    int breakpoint = subtxt.getLayout().getLineEnd(subrows-1);
		                    //Log.i("jaspal","breakpoint at :"+breakpoint);
		 
		                    Spannable s1 = new SpannableStringBuilder(spanS, 0, breakpoint);
		                    //Log.i("jaspal","s1 spannable is :"+Html.toHtml(s1));
		                    s1.setSpan(new Custom_LeadingMarginSpace(subrows, imgwdth+6),0, s1.length(),0);
		                    Spannable s2 = new SpannableStringBuilder(System.getProperty("line.separator"));
		                    Spannable s3 = new SpannableStringBuilder(spanS, breakpoint, spanS.length());
		                    // It is needed to set a zero-margin span on for the text under the image to prevent the space on the right!
		                    s3.setSpan(new Custom_LeadingMarginSpace(0,0),0, s3.length(),0);
		                    subtxt.setText(TextUtils.concat(s1, s2, s3));
		                } 
		 
		                // remove the GlobalLayoutListener 
		                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
		                     subtxt.getViewTreeObserver().removeOnGlobalLayoutListener(this);                           
		                 } else { 
		                     subtxt.getViewTreeObserver().removeGlobalOnLayoutListener(this);
		                 } 
		            } 
		        });
				
				//SpannableString subss = new SpannableString(object_SubNewsItem.getNewsContent());
				//SpannableString subss = new SpannableString(Html.fromHtml(object_SubNewsItem.getNewsContent()));
				//subss.setSpan(new Custom_LeadingMarginSpace(subrows, imgwdth+6), 0, subss.length(),  Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
...
//					hdg.setText(object_SubNewsItem.getNewsHeadingSpan());
				...
				ImageView img = (ImageView)singlesubNewsView.findViewById(R.id.img_newsimage);
				int wdth = (Globals.getScreenSize(this).x-20);
				int hgt = (int) (wdth*object_SubNewsItem.getImageRatio());
				
				img.setTag(R.id.img_newsimage,Integer.valueOf(i));
				
				LinearLayout.LayoutParams img_lp = new LinearLayout.LayoutParams(wdth,hgt);
				img.setLayoutParams(img_lp);
				
				//Globals.loadImageIntoImageView(img, object_SubNewsItem.getNewsImagePath(), this,R.drawable.loading_image_small,R.drawable.no_image_small);
				Globals.loadImageIntoImageView(img, object_SubNewsItem.getNewsImagePath(),this,R.drawable.loading, R.drawable.loading,hgt,wdth);
				TextView txt = (TextView)singlesubNewsView.findViewById(R.id.txt_newscontent);
				txt.setTypeface(tf);
			...
			}else if(object_SubNewsItem.getImageAlign() == Globals.IMAGE_ALIGN_RIGHT)
			{
				...
				subtxt_lp.setMargins(0, 0, imgwdth+6, 0);
				subtxt_imgside.setLayoutParams(subtxt_lp);
				subtxt_imgside.setText(object_SubNewsItem.getNewsContentSpan());
